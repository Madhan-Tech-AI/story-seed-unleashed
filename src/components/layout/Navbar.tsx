      ...
      <AnimatePresence>
        ...
        opacity: 0
            ...
            <div className="container mx-auto px-4 max-w-7xl py-2 flex items-center justify-center gap-4 relative">
              ...
              <button onClick={() => setIsBannerVisible(false)} className="absolute right-4 p-1 hover:bg-primary-foreground/20 rounded transition-colors" aria-label="Close announcement">
                <X className="w-4 h-4" />
              </button>
            </div>
          </motion.div>}
            ...
            <div className="bg-card rounded-xl p-4 shadow-lg space-y-2 max-h-[85vh] overflow-y-auto overscroll-contain">
              ...
              <div className="pt-4 border-t border-border space-y-2">
                ...
                {isAuthenticated && user && (role === 'admin' || role === 'judge') ? <>
                  ...
                  </> : <Button variant="default" className="w-full" onClick={handleGoogleSignIn} disabled={isGoogleSigningIn}>
                    {isGoogleSigningIn ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <User className="w-4 h-4 mr-2" />}
