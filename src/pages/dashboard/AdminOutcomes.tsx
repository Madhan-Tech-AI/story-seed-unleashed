  ...
  const fetchData = async () => {
    try {
      ...
      const { data: registrations, error } = await supabase
        .from('registrations')
        .select('id, story_title, first_name, last_name, overall_votes')
          ...
          {entries.length === 0 ? (
            ...
            <div className="space-y-3">
              {entries.map((entry, i) => (
                <div 
                  key={entry.id} 
                  className={`flex items-center gap-3 p-3 rounded-xl rank-rise ${
                    i === 0 ? 'bg-gradient-to-r from-secondary/20 to-accent/10' : 'bg-muted/30'
                  }`} 
